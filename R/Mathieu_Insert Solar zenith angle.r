...
hour_start=0
hour_end=24
hour=seq(from = hour_start, to = hour_end-1/60, by =1/60 )
n=length(hour)
seq.temps=seq(as.Date("2015-07-15", tz = "RET"),as.Date("2015-07-16",tz = "RET"),by="day")
...
for (i in 1:length(seq.temps)){
  ...
  x <- AstroCalcPureR(day=rep(day,n),month=rep(month,n),year=rep(year,n),hour=hour,timezone=rep(4,n),lat=rep(-20.90307,n),lon=rep(55.48442,n))
  seq.date=seq(as.POSIXct(seq.temps[i])-4*60*60,as.POSIXct(seq.temps[i]+1)-4*60*60-1*60,by="min")
  AstroVals=rbind(AstroVals,cbind(as.data.frame(seq.date),x)) 
}
