...
hour_start=0
hour_end=24
hour=seq(from = hour_start, to = hour_end-6, by =6)
n=length(hour)
seq.temps=seq(as.Date("2001-01-01", tz = "RET"),as.Date("2005-12-30",tz = "RET"),by="month")
...
for (i in 1:length(seq.temps)){
  ...
  seq.date=seq(as.POSIXct(seq.temps[i])-4*60*60,as.POSIXct(seq.temps[i]+1)-4*60*60-1*60,by="month")
